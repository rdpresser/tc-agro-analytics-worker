namespace TC.Agro.Analytics.Service.Middleware;
...
/// - Record system and analytics metrics
...
public class TelemetryMiddleware
{
    ...
    private readonly AnalyticsMetrics _analyticsMetrics;
    ...
    public async Task InvokeAsync(HttpContext context, ICorrelationIdGenerator correlationIdGenerator)
    {
        ...
        using (LogContext.PushProperty("user.authenticated", isAuthenticated))
        {
            ...
            try
            {
                // Record analytics action if authenticated
                if (isAuthenticated)
                {
                    _analyticsMetrics.RecordAnalyticsAction($"{context.Request.Method}", userId, path);
                }
