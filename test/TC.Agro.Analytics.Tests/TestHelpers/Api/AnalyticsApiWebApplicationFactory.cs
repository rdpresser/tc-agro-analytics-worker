...
namespace TC.Agro.Analytics.Tests.TestHelpers.Api;

public sealed class AnalyticsApiWebApplicationFactory : WebApplicationFactory<AnalyticsProgram>
{
    private readonly string _databaseName = $"analytics-api-tests-{Guid.NewGuid():N}";

    public AnalyticsApiWebApplicationFactory()
    {
    ...
    public async Task<Guid> SeedPendingAlertAsync(Guid ownerId, Guid sensorId)
    {
        await using var scope = Services.CreateAsyncScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();

        dbContext.OwnerSnapshots.Add(OwnerSnapshot.Create(ownerId, "Owner API", $"{ownerId:N}@owner.test"));
        ...
        var alertResult = AlertAggregate.Create(
            ...
            message: "High temperature detected: 42.5C",
            value: 42.5,
            threshold: 35.0);

        alertResult.IsSuccess.ShouldBeTrue();

        dbContext.Alerts.Add(alertResult.Value);

        await dbContext.SaveChangesAsync().ConfigureAwait(false);

        return alertResult.Value.Id;
    }
    ...
    private static class TestConfiguration
    {
        public static readonly IReadOnlyDictionary<string, string?> Values = new Dictionary<string, string?>
        {
            ...
            ["Database:Postgres:Database"] = "analytics-tests",
            ...
            ["Cache:Redis:InstanceName"] = "analytics-api-tests",
            ...
            ["Messaging:RabbitMQ:Exchange"] = "analytics.events",
            ...
            ["Alerts:Thresholds:MaxTemperature"] = "35",
            ["Alerts:Thresholds:MinSoilMoisture"] = "30",
            ["Alerts:Thresholds:MinBatteryLevel"] = "20"
        };
    }
